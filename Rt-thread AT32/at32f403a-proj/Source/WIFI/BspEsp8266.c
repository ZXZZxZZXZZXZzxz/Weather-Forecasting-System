//根据 <野火电子> ESP8266模块驱动修改
#include "config.h"
#include "BspEsp8266.h"
 
bool WIFI_connect_Sign;     //连接状态
...
//											   
//	GPIO_InitStructure.GPIO_Pins = macESP8266_CH_PD_PIN;	
...
//	GPIO_Init ( macESP8266_CH_PD_PORT, & GPIO_InitStructure );	 
	  ...
	  rt_pin_mode(macESP8266_CH_PD_PIN, PIN_MODE_OUTPUT);

//	/* 配置 RST 引脚*/
//	macESP8266_RST_APBxClock_FUN ( macESP8266_RST_CLK, ENABLE ); 
											   
//	GPIO_InitStructure.GPIO_Pins = macESP8266_RST_PIN;	

//	GPIO_Init ( macESP8266_RST_PORT, & GPIO_InitStructure );	 
    rt_pin_mode(macESP8266_RST_PIN, PIN_MODE_OUTPUT);
}

/**
  * @brief  初始化ESP8266用到的 USART2
  * @param  无
  * @retval 无
  */
static void ESP8266_USART_Config ( void )
{
	...
	GPIO_InitStructure.GPIO_Pins =  macESP8266_USART_TX_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;
	GPIO_Init(macESP8266_USART_TX_PORT, &GPIO_InitStructure);  
  
	/* Configure USART Rx as input floating */
	GPIO_InitStructure.GPIO_Pins = macESP8266_USART_RX_PIN;
	...
	/* 中断配置 */
	USART_INTConfig ( macESP8266_USARTx, USART_INT_RDNE, ENABLE ); //使能串口接收中断 
	USART_INTConfig ( macESP8266_USARTx, USART_INT_IDLEF, ENABLE ); //使能串口总线空闲中断 	
...
bool ESP8266_JoinAP ( char * pSSID, char * pPassWord )
{
	...
	return ESP8266_Cmd ( cCmd, "OK", NULL, 6000 );
...
bool ESP8266_Link_Server ( ENUM_NetPro_TypeDef enumE, char * ip, char * ComNum, ENUM_ID_NO_TypeDef id)
{
  ...
  if ( id < 5 )
	...
	return ESP8266_Cmd ( cCmd, "CONNECT", "OK", 4000 );
...
/************************************************************************/
#if BSP_IF_USING_ESP8266_USART2_IRQHandler
void macESP8266_USART_INT_FUN ( void )
{	
	uint8_t ucCh;
	
	if ( USART_GetITStatus ( macESP8266_USARTx, USART_INT_RDNE ) != RESET )
	{
	...
	if ( USART_GetITStatus( macESP8266_USARTx, USART_INT_IDLEF ) == SET )                                         //数据帧接收完毕
	{
